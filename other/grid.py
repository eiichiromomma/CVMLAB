...
if not is_win32:
       ##momma##WindowsのPythonを使うので、この中身はどうでも良い
       svmtrain_exe = "/cygdrive/c/local/libsvm-2.89/windows/svm-train"
       gnuplot_exe = "/usr/bin/gnuplot"
else:
       # example for windows
       ##momma##ローカルマシン向けのフルパス
       svmtrain_exe = r"c:\\local\\libsvm-2.89\\windows\\svm-train.exe"
       gnuplot_exe = r"c:\\local\\gnuplot\\bin\\pgnuplot.exe"
...
telnet_workers = []
##momma##sshで利用するマシンのList。1つにつき1プロセスしか実行されないので、
##momma##プロセッサ数だけマシン名を羅列すると良い
ssh_workers = ["host2", "host2", "host3","host3","host3","host3","host3","host3", "host4","host4","host4","host4","host4","host4", "host5","host5","host5","host5","host5","host5",]
##momma##ローカルマシンのプロセッサ数
nr_local_worker = 4
...
class SSHWorker(Worker):
    def __init__(self,name,job_queue,result_queue,host):
        ...
        self.host = host
        ##momma##os.getcwdだとc:\local...の形式で取得してしまい、
        ##momma##sshでログインしたcygwinのシステムでは\が無視されてエラーになるので決め打ち
        ##momma##(cygpathを使えばもっと綺麗に実装できそう)
        ##momma##svm-train.exeの場所をcygwin形式で記述する。
        ##momma##self.cwd = os.getcwd()
